...
# Read data
print('reading data...')
data_file = 'law_docs.txt'
with open(data_file, 'r', encoding="utf-8") as f:
    docs = f.readlines()
    train_data = [list(a.split(' ')) for a in docs]
...
init_docs_tr = train_data[:int(len(train_data)*0.8)]
init_docs_ts = train_data[int(len(train_data)*0.8):]
init_docs = train_data
...
docs_va = [[word2id[w] for w in init_docs[idx_d+num_docs_tr].split() if w in word2id] for idx_d in range(tsSize)][:vaSize]
